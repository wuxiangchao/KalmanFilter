...
class KalmanFilter:
    def __init__(self,delta_t, sigma_a, sigma_x, sigma_y):
        ...
        self.Q = np.matrix(np.array(
        	[
        		[self.delta_t**4/4,   self.delta_t**3/2,  self.delta_t**2/2,   0, 0, 0],
        		[self.delta_t**3/2,   self.delta_t**2, self.delta_t,     0, 0, 0],
        		[self.delta_t**2/2, self.delta_t**2,1,0,0,0],
        		[0,0,0,self.delta_t**4/4,self.delta_t**3/2,self.delta_t**2/2],
    ...
    def update(self, z):
        self.Kn = self.P.dot(self.H.transpose()).dot( (self.H.dot(self.P).dot(self.H.transpose()) + self.R).I)
